...
genome = read_gff3("../Data/Actinia_equina_gca011057435.equina_smartden.arrow4.noredun.dna.nonchromosomal.fa.gz")
...
# 9. Generate manhattan plots of overlapped Results Data ----
...
plot <- plot(act_diff_CpG_data$meth.diff, (-log10(act_diff_CpG_data$pvalue)), 
             xlab="Methylation Difference (%)", 
             ylab="-log10(Adjusted P-Value)", 
             main="Differential Methylation Scatterplot")

ggplot(data = act_diff_CpG_data,
       aes(x = meth.diff, y = -log10(pvalue))) + 
  geom_point(size = 3) + 
  geom_hline(yintercept = c(1), col = "black", linetype = "dashed") + 
  theme_bw() + 
  labs(x = "Methylation percentage (%) Difference",
       y = expression("-log"[10]*"p value"),
       title = "Differential Methylation Volcano plot of Experiment 1")
